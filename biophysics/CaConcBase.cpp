...
{
    static SrcFinfo1<double> concOut("concOut", "Concentration of Ca in pool");
    return &concOut;
}
...
const Cinfo* CaConcBase::initCinfo()
{
    ...
    static SharedFinfo proc(
        "proc", "Shared message to receive Process message from scheduler",
        processShared, sizeof(processShared) / sizeof(Finfo*));

    ///////////////////////////////////////////////////////
    // Field definitions
    ///////////////////////////////////////////////////////
    static ElementValueFinfo<CaConcBase, double> Ca(
        "Ca", "Calcium concentration.", &CaConcBase::setCa, &CaConcBase::getCa);
    static ElementValueFinfo<CaConcBase, double> CaBasal(
        "CaBasal", "Basal Calcium concentration.", &CaConcBase::setCaBasal,
        &CaConcBase::getCaBasal);
    static ElementValueFinfo<CaConcBase, double> Ca_base(
        "Ca_base", "Basal Calcium concentration, synonym for CaBasal",
        &CaConcBase::setCaBasal, &CaConcBase::getCaBasal);
    static ElementValueFinfo<CaConcBase, double> tau(
        "tau", "Settling time for Ca concentration", &CaConcBase::setTau,
        &CaConcBase::getTau);
    static ElementValueFinfo<CaConcBase, double> B(
        "B",
        ...
        "dC/dt = B*I_Ca - C/tau",
        &CaConcBase::setB, &CaConcBase::getB);
    static ElementValueFinfo<CaConcBase, double> thick(
        "thick",
        ...
        "\n",
        &CaConcBase::setThickness, &CaConcBase::getThickness);
    static ElementValueFinfo<CaConcBase, double> length(
        "length", "Length of Ca shell, assumed cylindrical",
        &CaConcBase::setLength, &CaConcBase::getLength);
    static ElementValueFinfo<CaConcBase, double> diameter(
        "diameter", "Diameter of Ca shell, assumed cylindrical",
        &CaConcBase::setDiameter, &CaConcBase::getDiameter);
    static ElementValueFinfo<CaConcBase, double> ceiling(
        "ceiling",
        ...
        &CaConcBase::setCeiling, &CaConcBase::getCeiling);
    static ElementValueFinfo<CaConcBase, double> floor(
        "floor", "Floor value for Ca concentration. If Ca < floor, Ca = floor",
        &CaConcBase::setFloor, &CaConcBase::getFloor);

    ///////////////////////////////////////////////////////
    // MsgDest definitions
    ///////////////////////////////////////////////////////

    static DestFinfo current(
        "current", "Calcium Ion current, due to be converted to conc.",
        new EpFunc1<CaConcBase, double>(&CaConcBase::current));

    static DestFinfo currentFraction(
        "currentFraction",
        "Fraction of total Ion current, that is carried by Ca2+.",
        new EpFunc2<CaConcBase, double, double>(&CaConcBase::currentFraction));

    static DestFinfo increase(
        "increase", "Any input current that increases the concentration.",
        new EpFunc1<CaConcBase, double>(&CaConcBase::increase));

    static DestFinfo decrease(
        "decrease", "Any input current that decreases the concentration.",
        new EpFunc1<CaConcBase, double>(&CaConcBase::decrease));

    static DestFinfo basal(
        "basal", "Synonym for assignment of basal conc.",
    ...
    static Cinfo CaConcBaseCinfo("CaConcBase", Neutral::initCinfo(),
                                 CaConcBaseFinfos,
...
CaConcBase::CaConcBase() : thickness_(0.0)
{
    ;
}
...
void CaConcBase::updateDimensions(const Eref& e)
{
    double vol{0};
    if(length_ > 0) {
        vol = PI * diameter_ * diameter_ * length_ * 0.25;
    }
    else {
        vol = PI * diameter_ * diameter_ * diameter_ * 0.5 / 3;
    }
    if(thickness_ > 0 && thickness_ < diameter_ / 2.0) {
        ...
        if(length_ > 0) {
            ...
            vol -= 4 * PI * coreRadius * coreRadius * coreRadius / 3;
        }
    }
