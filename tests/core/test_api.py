...
def test_other():
    ...
    finfo = moose.getFieldTypeDict(a1.className)
...
def test_vec():
    ...
    v = moose.vec(a)
    vv = moose.element(a).vec   # ObjId.vec returns the MooseVec
    ...
    assert v == vv
    assert v[0] == vv[0], (v[0], vv[0])
    assert v[1] == vv[1], (v[1], vv[1])
...
def test_vec2():
    ...
    vv = moose.element('/p1112').vec   # ObjId.vec returns the MooseVec
    # le can cause segfault in some cases.
    moose.le(v)
